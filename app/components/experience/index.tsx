...
const Experience = () => {
  ...
  useFrame((sate, delta) => {
    ...
    if (titleRef.current) {
      titleRef.current.visible = !isActive;
      if (!isActive) {
        titleRef.current.children.forEach((text, i) => {
          const y =  Math.max(Math.min((1 - d) * (13 - i), 13), 0.5);
  ...
  const getTitle = () => {
    const title = 'my experience'.toUpperCase();
    return title.split('').map((char, i) => {
      const diff = isMobile ? 0.32 : 0.65;
      return (
  ...
  return (
    <group position={[0, -41.5, 12]} rotation={[-Math.PI / 2, 0 ,-Math.PI / 2]}>
      ...
      <group rotation={[0, 0, Math.PI / 2]}>
        <group ref={titleRef} position={[isMobile ? -1.92 : -3.9, 2, -2]}>
          {getTitle()}
        </group>
